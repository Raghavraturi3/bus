...
// ---------------------- DEMO WITH ORIGIN + DESTINATION ----------------------
export default function DemoSelect() {
  const allStops = ["Dehradun", "Rishikesh", "Haridwar", "Mussoorie"];
  const [fromStop, setFromStop] = React.useState<string>("");
  const [toStop, setToStop] = React.useState<string>("");
  ...
  return (
    <div className="space-y-4">
      ...
      <Select value={toStop} onValueChange={setToStop}>
        ...
        <SelectContent>
          {allStops
            .filter((stop) => stop !== fromStop) // origin ka option hide kar diya
            .map((stop) => (
              <SelectItem key={stop} value={stop}>
                {stop}
              </SelectItem>
